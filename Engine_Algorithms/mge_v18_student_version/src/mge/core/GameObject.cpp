...
bool GameObject::Collides(GameObject * pOtherObject)
{
	if (_collider && pOtherObject) {
	//	hit = _collider->Collides(pOtherObject->getCollider()); //Double Dispatch
		hit = _collider->CollidesWith(pOtherObject->getCollider()); //Single Dispatch
		if (hit) {
			this->setMaterial(_hitMaterial);
			pOtherObject->setMaterial(_hitMaterial);
		}
		return hit;
	}
	else {
...
void  GameObject::SetHitMaterial(AbstractMaterial* pMaterial) {
	_hitMaterial = pMaterial;
}
...
void GameObject::update(float pStep)
{
	...
	if (_collider) {
		_collider->SetCenter(getWorldPosition());
	}
